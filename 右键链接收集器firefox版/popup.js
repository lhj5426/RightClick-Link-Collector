...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function setCount() {
    countLine.textContent = `已保存 ${allLinks.length} 个链接`;
  }
  ...
  function render() {
    ...
    visible.forEach((link) => {
      ...
      copyBtn.className = "small-btn copy-small";
  ...
  function copySingle(link, itemElem) {
    navigator.clipboard.writeText(link.url).then(() => {
      ...
      setTimeout(() => {
        msg.remove();
        itemElem.classList.remove("glow");
      }, 1400);
    }).catch((err) => {
  ...
  return `      <div class="link-item${favoriteClass}">
    ...
    if (format === "doc") {
      ...
      allLinks.forEach((l, i) => {
        ...
        docParts.push(
          `<p style="font-family:Calibri,Arial;"><b>${i + 1}. ${safeTitle}</b><br><a href="${safeUrl}" style="color:#0645AD;text-decoration:underline;">${safeUrl}</a><br>${safeDate}<br>收藏: ${favText}</p>`
        );
      });
